...
4. Aspen Adsorption 中的 **Extended Langmuir 2** 模型通常使用 IP_1 到 IP_4 四个参数。
...
在吸附等温线参数拟合阶段，尽管使用的是扩展型模型框架，但因实验数据源于 **单组分测量** ，模型分母中描述竞争吸附的 **求和项会自动退化** （即仅含当前单一组分参数）。然而，在推导至 **Aspen Adsorption** 或 **MATLAB** 的 PSA 循环模拟时，必须逻辑性地还原多组分竞争本质。以

```
PSA_matlab\code_cn\CycleSteps\Isotherm.m
```

 中的实现为例，程序在计算吸附量时，需将单组分拟合得到的动力学参数重新代入耦合公式，通过**分母求和项**将混合气中两组分（或多组分）的压力贡献与亲和力系数同时纳入计算，从而准确模拟实际变压吸附过程中组分间的竞争效应。
