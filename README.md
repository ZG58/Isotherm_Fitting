...
Aspen Adsorption 中的 **Extended Langmuir 2** 模型通常使用  到  四个参数。
...
* Aspen 的默认单位集（SI）中，吸附量通常是 **** (即 )。
* 压力通常是 。


3. **如何输入到 Aspen**：
* **方法 A（推荐）：修改 Aspen 单位集**
在 Aspen Adsorption 的单位设置（Unit Set）中，将 `Mole loading` 或组分容量单位显式设置为 ****。此时，你可以**直接输入** CSV 文件中的  值作为 。
* **方法 B：手动换算**
如果保持 Aspen 默认的 **** 单位，你需要将 CSV 文件中的  值除以 1000 后再输入到 。


* * (即 ) 的单位与  无关，无需转换，直接输入即可。*
